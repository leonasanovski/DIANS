...
# Следните 3 функции се дел од „ФИЛТЕР 2“, каде соодветно ќе запише податоци во база за сите 10 години, а потоа со филтер 3 ќе ги дозапишува новитетите
def get_last_recorded_date(company_code, csv_file):
    ...
    # If no data exists, return a date from 10 years ago
...
def get_date_ranges_for_update(company_code, csv_file):
    ...
    # Create a list of date ranges in one-year intervals
    date_ranges = [(f"1/1/{year}", f"12/31/{year}") for year in range(start_year, end_year + 1)]
    date_ranges[-1] = (f"1/1/{end_year}", datetime.now().strftime("%m/%d/%Y"))
...
async def save_to_csv(rows):
    ...
    new_data["Датум"] = pd.to_datetime(new_data["Датум"])

    if not existing_data.empty:
        existing_data["Датум"] = pd.to_datetime(existing_data["Датум"])
...
# Тука е повикувањето на сите филтри и соодветно склопот на целата програма
async def main():
